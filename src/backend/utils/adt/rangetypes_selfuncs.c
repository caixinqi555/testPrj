...
static double clamp_hist_extreme_selectivity(TypeCacheEntry *typcache,
											 ...
											 int hist_nvalues,
											 double selec);
static double clamp_range_hist_result(TypeCacheEntry *typcache,
					  ...
					  const RangeType *constval, Oid operator)
{
	...
	return clamp_range_hist_result(typcache, &const_lower, &const_upper,
...
/*
 * Keep composed range estimates away from exact 0/1 when any finite query
 * bound lies outside the bound histograms.  This catches cases like "&&",
 * where complementary scalar estimates can still combine to an exact extreme.
 */
static double
clamp_range_hist_result(TypeCacheEntry *typcache,
						  const RangeBound *const_lower,
						  const RangeBound *const_upper,
						  const RangeBound *hist_lower,
						  const RangeBound *hist_upper,
								...
								const Datum *length_hist_values, int length_hist_nvalues)
{
	...
	/*
	 * If we estimated no matches but the constant lower bound lies strictly
	 * above the histogram's largest stored lower bound, the iterative
	 * calculation above can drive bin_width to zero even though real data
	 * may extend past the histogram endpoint (histograms are not guaranteed
	 * to reach the true column maximum).  Treating that tail as empty
	 * yields a hard zero row estimate which is usually too pessimistic for
	 * the planner.  Fall back to the same default we use when no histogram
	 * is available.
	 */
	if (sum_frac <= 0.0 &&
		range_cmp_bounds(typcache, lower, &hist_lower[hist_nvalues - 1]) > 0)
		sum_frac = default_range_selectivity(OID_RANGE_CONTAINED_OP);
