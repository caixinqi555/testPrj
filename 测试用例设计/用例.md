...
CALL check_erows('R-A-01', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 100, 0.05);

-- 【R-A-02】range 触发 | astore | 单列 | 上界截断 | int
-- 预期：触发 range 边界外矫正 | rows ≈ 2000（±5%）| 公式 min(9000*1.0=9000, increase_tuples≈2000) = 2000
CALL check_erows('R-A-02', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 20000$$, 2000, 0.05);

-- 【R-A-03】range 触发 | ustore | 单列 | 公式值生效 | int
-- 预期：触发 range 边界外矫正 | rows ≈ 100（±5%）| 公式 min(100, 2000) = 100
CALL check_erows('R-A-03', $$SELECT * FROM t_r_ustore WHERE id > 11000 AND id < 11100$$, 100, 0.05);

-- 【R-A-04】range 触发 | ustore | 单列 | 上界截断 | int
-- 预期：触发 range 边界外矫正 | rows ≈ 2000（±5%）| 公式 min(9000, 2000) = 2000
CALL check_erows('R-A-04', $$SELECT * FROM t_r_ustore WHERE id > 11000 AND id < 20000$$, 2000, 0.05);

-- 【R-A-05】range 触发 | 一级分区-剪枝单分区 p11 | 单列 | 公式值生效 | int
-- 预期：触发 range 边界外矫正 | 在分区 p11 上估行 ≈ 100（±5%）| 走分区本地统计信息
CALL check_erows('R-A-05', $$SELECT * FROM t_r_part1 WHERE id > 11000 AND id < 11100$$, 100, 0.05);

-- 【R-A-06】range 触发 | 一级分区-剪枝多分区 p11+p12 | 单列 | 上界截断 | int
-- 预期：触发 range 边界外矫正 | rows ≈ 2000（±5%）| 公式值远大于 increase_tuples，剪枝后上界截断
CALL check_erows('R-A-06', $$SELECT * FROM t_r_part1 WHERE id > 10000 AND id < 14000$$, 2000, 0.05);
...
CALL check_erows('R-A-07', $$SELECT * FROM t_r_part1 WHERE ts > '2026-01-02 00:00:00' AND ts < '2026-01-02 00:01:40'$$, 100, 0.05);

-- 【R-A-08】range 触发 | 一级分区-不剪枝 | 单列 | 上界截断 | int
-- 预期：触发 range 边界外矫正 | rows ≈ 2000（±5%）| 公式值远大于 increase_tuples，上界截断
CALL check_erows('R-A-08', $$SELECT * FROM t_r_part1 WHERE ts > '2026-01-02 00:00:00' AND ts < '2026-01-10 00:00:00'$$, 2000, 0.05);
```
...
-- 预期：不触发矫正（原逻辑）| rows 按直方图估算（≈ 几百，非 1）| qual_l=9000<stats_max=10000，不满足触发条件
CALL check_erows('R-B-01', $$SELECT * FROM t_r_astore WHERE id > 9000 AND id < 11000$$, 1000, 0.05);

-- 【R-B-02】range 不触发 | 谓词范围完全包含在统计信息范围内
-- 预期：不触发矫正（原逻辑）| rows 按直方图估算（≈ 200）| 谓词两端均在边界内
CALL check_erows('R-B-02', $$SELECT * FROM t_r_astore WHERE id > 5000 AND id < 5200$$, 200, 0.05);
```
...
CALL check_erows('R-C-01', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 1, 0.05);
...
ANALYZE t_r_astore;
CALL check_erows('R-C-02', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100 AND ts > '2026-04-01'::timestamp$$, 1, 0.05);
DROP STATISTICS st_r_astore_ts_id;

-- 【R-C-03】range 约束回退 | enable_static_statistics=on
-- 预期：不触发矫正（整体不支持）| rows = 1（原逻辑）
SET enable_static_statistics = on;
CALL check_erows('R-C-03', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 1, 0.05);
SET enable_static_statistics = off;
...
CALL check_erows('R-C-04', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 100, 0.05);
...
CALL check_erows('R-C-05', $$SELECT * FROM t_r_nopagegrow WHERE id > 11000 AND id < 11100$$, 1, 0.05);
```
...
CALL check_erows('R-D-01', $$SELECT * FROM t_r_temp WHERE id > 11000 AND id < 11100$$, 100, 0.05);
...
CALL check_erows('R-D-02', $$SELECT * FROM t_r_gtt WHERE id > 11000 AND id < 11100$$, 100, 0.05);

-- 【R-D-03】统计信息类型 | 表达式统计信息（range 谓词在表达式列上）
-- 预期：触发 range 边界外矫正 | rows ≈ 100（±5%）| 表达式列走非索引主列路径
CREATE STATISTICS st_r_expr ON (id * 2) FROM t_r_astore;
ANALYZE t_r_astore;
CALL check_erows('R-D-03', $$SELECT * FROM t_r_astore WHERE (id * 2) > 22000 AND (id * 2) < 22200$$, 100, 0.05);
DROP STATISTICS st_r_expr;
...
CALL check_erows('R-D-04', $$SELECT * FROM t_r_noincr WHERE id > 11000 AND id < 11100$$, 1, 0.05);

-- 【R-D-05】历史统计信息已 prune（回退 estimate_rel_size 当前值）
-- 预期：触发 range 边界外矫正 | rows ≈ 100（±容忍，由当前 relpages/reltuples 代替历史值）
-- 前置：手动清理历史统计信息表中 t_r_astore 相关记录 或 等待 prune 周期
DELETE FROM pg_statistic_history WHERE starelid = 't_r_astore'::regclass;  -- 伪代码，视 GaussDB 实际表名
CALL check_erows('R-D-05', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 100, 0.05);

-- 【R-D-06】vacuum 只更表级不更列级
-- 预期：触发 range 边界外矫正 | rows ≈ 100（±5%）| vacuum 后表级统计新了但列级直方图仍为旧值
VACUUM t_r_astore;   -- 更新 pg_class.relpages/reltuples，不更新 pg_statistic 列级统计
CALL check_erows('R-D-06', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 100, 0.05);
```
...
CALL check_erows('G-A-01', $$SELECT * FROM t_r_astore WHERE id > 11600$$, 400, 0.05);

-- 【G-A-02】gt 触发 | astore | 单列 | 上界截断 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 2000（±5%）| qual_l=10001 极近边界，外推宽度≈1999，结果贴近上界
CALL check_erows('G-A-02', $$SELECT * FROM t_r_astore WHERE id > 10001$$, 2000, 0.05);

-- 【G-A-03】gt 触发 | ustore | 单列 | 公式值生效 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 400（±5%）| 公式同 G-A-01
CALL check_erows('G-A-03', $$SELECT * FROM t_r_ustore WHERE id > 11600$$, 400, 0.05);

-- 【G-A-04】gt 触发 | ustore | 单列 | 上界截断 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 2000（±5%）| qual_l 靠近边界，估行结果贴近上界
CALL check_erows('G-A-04', $$SELECT * FROM t_r_ustore WHERE id > 10001$$, 2000, 0.05);

-- 【G-A-05】gt 触发 | 一级分区-剪枝到 p12 | 单列 | 公式值生效 | int
-- 预期：触发 gt 边界外矫正 | 在 p12 上估行 ≈ 400（±5%）
CALL check_erows('G-A-05', $$SELECT * FROM t_r_part1 WHERE id > 11600 AND id < 12001$$, 400, 0.05);

-- 【G-A-06】gt 触发 | 一级分区-剪枝到 p11+p12 | 单列 | 上界截断 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 2000（±5%）| qual_l 靠近边界，估行结果贴近上界
CALL check_erows('G-A-06', $$SELECT * FROM t_r_part1 WHERE id > 10001 AND id < 12001$$, 2000, 0.05);

-- 【G-A-07】gt 触发 | 一级分区-不剪枝 | 单列 | 公式值生效 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 400（±5%）| 走主表统计+外推
CALL check_erows('G-A-07', $$SELECT * FROM t_r_part1 WHERE ts > '2026-01-01 03:13:20'$$, 400, 0.05);

-- 【G-A-08】gt 触发 | 一级分区-不剪枝 | 单列 | 上界截断 | int
-- 预期：触发 gt 边界外矫正 | rows ≈ 2000（±5%）| qual_l 靠近边界，估行结果贴近上界
CALL check_erows('G-A-08', $$SELECT * FROM t_r_part1 WHERE ts > '2026-01-01 02:46:41'$$, 2000, 0.05);
```
...
-- 预期：不触发矫正（原逻辑）| rows 按直方图估算 ≈ 2000 | qual_l=8000<stats_max，不满足触发条件
CALL check_erows('G-B-01', $$SELECT * FROM t_r_astore WHERE id > 8000$$, 2000, 0.05);

-- 【G-B-02】gt 不触发 | qual_l 在统计信息范围内（恰好等于 stats_max）
-- 预期：不触发矫正（原逻辑）| rows 按直方图估算 | qual_l=10000 恰等 stats_max，不严格大于
CALL check_erows('G-B-02', $$SELECT * FROM t_r_astore WHERE id > 10000$$, 1, 0.05);
```
...
CALL check_erows('G-C-01', $$SELECT * FROM t_r_astore WHERE id > 10100$$, 1, 0.05);
...
ANALYZE t_r_astore;
CALL check_erows('G-C-02', $$SELECT * FROM t_r_astore WHERE id > 10100 AND ts > '2026-04-01'::timestamp$$, 1, 0.05);
DROP STATISTICS st_g_ts_id;

-- 【G-C-03】gt 约束回退 | enable_static_statistics=on
-- 预期：不触发矫正 | rows = 1（原逻辑）
SET enable_static_statistics = on;
CALL check_erows('G-C-03', $$SELECT * FROM t_r_astore WHERE id > 10100$$, 1, 0.05);
SET enable_static_statistics = off;

-- 【G-C-04】gt 约束回退 | 页面空洞
-- 预期：增量估算失真但不崩溃 | 本特性不探测空洞
DELETE FROM t_r_astore WHERE id BETWEEN 3000 AND 9000;
CALL check_erows('G-C-04', $$SELECT * FROM t_r_astore WHERE id > 10100$$, 1900, 0.05);
...
CALL check_erows('G-C-05', $$SELECT * FROM t_g_nopagegrow WHERE id > 10100$$, 1, 0.05);
```
...
CALL check_erows('G-D-01', $$SELECT * FROM t_g_temp WHERE id > 11600$$, 400, 0.05);
...
CALL check_erows('G-D-02', $$SELECT * FROM t_g_gtt WHERE id > 11600$$, 400, 0.05);

-- 【G-D-03】表类型 | 一级分区-剪枝多
-- 预期：触发 gt 边界外矫正 | rows ≈ 1900（±5%）| 剪枝到 p11+p12 两个分区
CALL check_erows('G-D-03', $$SELECT * FROM t_r_part1 WHERE id > 10100 AND id < 12001$$, 1900, 0.05);
...
CALL check_erows('G-D-04', $$SELECT * FROM t_g_part2 WHERE id > 10100$$, 1900, 0.05);  -- 只按 id 过滤，二级维度不剪枝

-- 【G-D-05】表类型 | 二级分区-剪枝到叶子
-- 预期：触发 gt 边界外矫正 | 剪枝到单个叶子分区 p03_r1
CALL check_erows('G-D-05', $$SELECT * FROM t_g_part2 WHERE id > 10100 AND region = 1$$, 950, 0.05);

-- 【G-D-06】表类型 | 二级分区-不剪枝
-- 预期：触发 gt 边界外矫正 | 无法剪枝，走主表统计+存储层访问
CALL check_erows('G-D-06', $$SELECT * FROM t_g_part2 WHERE v > 'z9999'$$, 1, 0.05);  -- v 非分区键，无法剪枝

-- 【G-D-07】统计信息类型 | 表达式统计信息
-- 预期：触发 gt 边界外矫正 | 表达式列走非索引主列路径
CREATE STATISTICS st_g_expr ON (id * 2) FROM t_r_astore;
ANALYZE t_r_astore;
CALL check_erows('G-D-07', $$SELECT * FROM t_r_astore WHERE (id * 2) > 20200$$, 1900, 0.05);
DROP STATISTICS st_g_expr;
```
...
CALL check_erows('E-A-01', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 200, 0.05);
...
CALL check_erows('E-A-02', $$SELECT * FROM t_e_astore_low WHERE ver = 3$$, 2000, 0.05);
...
ANALYZE t_e_astore;
CALL check_erows('E-A-03', $$SELECT * FROM t_e_astore WHERE ver = 51 AND region = 3$$, 20, 0.05);
...
ANALYZE t_e_astore_low;
CALL check_erows('E-A-04', $$SELECT * FROM t_e_astore_low WHERE ver = 7$$, 2000, 0.05);
DROP STATISTICS st_e_ver_region2;

-- 【E-A-05】equal 触发 | ustore | 单列 | 未命中+other=0 | 公式值生效
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±5%）
CALL check_erows('E-A-05', $$SELECT * FROM t_e_ustore WHERE ver = 51$$, 200, 0.05);
...
CALL check_erows('E-A-06', $$SELECT * FROM t_e_ustore_low WHERE ver = 3$$, 2000, 0.05);

-- 【E-A-07】equal 触发 | ustore | 多列 | 公式值生效
CREATE STATISTICS st_e_ust_mc ON ver, region FROM t_e_ustore;
ANALYZE t_e_ustore;
CALL check_erows('E-A-07', $$SELECT * FROM t_e_ustore WHERE ver = 51 AND region = 3$$, 20, 0.05);
...
ANALYZE t_e_ustore_low;
CALL check_erows('E-A-08', $$SELECT * FROM t_e_ustore_low WHERE ver = 7$$, 2000, 0.05);
DROP STATISTICS st_e_ust_mc2;

-- 【E-A-09】equal 触发 | 一级分区-剪枝单（p3）| 单列 | 公式值生效
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±5%）| p3 上 ver=51 未命中该分区 MCV
CALL check_erows('E-A-09', $$SELECT * FROM t_e_part1 WHERE ver = 51 AND region = 8$$, 200, 0.05);
...
ANALYZE t_e_part1_low;
CALL check_erows('E-A-16', $$SELECT * FROM t_e_part1_low WHERE ver = 6 AND region IN (1,5,9)$$, 600, 0.05);
...
-- 预期：不触发矫正（原逻辑）| rows ≈ 200（按 MCV 频率）| ver=1 是 MCV 值
CALL check_erows('E-B-01', $$SELECT * FROM t_e_astore WHERE ver = 1$$, 200, 0.05);
...
CALL check_erows('E-B-02', $$SELECT * FROM t_e_otherdist WHERE ver = 50$$, 10, 0.05);  -- 未命中 MCV，但 other_distinct>0，走原逻辑
```

### 5.4 组 C：约束回退 3 条

```sql
-- 【E-C-01】equal 约束回退 | enable_static_statistics=on
-- 预期：不触发矫正（整体不支持）| rows = 1（原逻辑）
SET enable_static_statistics = on;
CALL check_erows('E-C-01', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 1, 0.05);
SET enable_static_statistics = off;

-- 【E-C-02】equal 约束回退 | 页面空洞
-- 预期：估算失真但不崩溃
DELETE FROM t_e_astore WHERE ver BETWEEN 10 AND 40;
CALL check_erows('E-C-02', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 200, 0.05);
...
CALL check_erows('E-C-03', $$SELECT * FROM t_e_nopagegrow WHERE ver = 51$$, 1, 0.05);
```
...
CALL check_erows('E-D-01', $$SELECT * FROM t_e_temp WHERE ver = 51$$, 200, 0.05);
...
CALL check_erows('E-D-02', $$SELECT * FROM t_e_gtt WHERE ver = 51$$, 200, 0.05);

-- 【E-D-03】表类型 | 一级分区-剪枝多分区
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±5%）| 剪枝到 p2+p3
CALL check_erows('E-D-03', $$SELECT * FROM t_e_part1 WHERE ver = 51 AND region >= 4$$, 200, 0.05);
...
CALL check_erows('E-D-04', $$SELECT * FROM t_e_part2 WHERE ver = 51 AND region = 8$$, 200, 0.05);

-- 【E-D-05】表类型 | 二级分区-剪枝到叶子
-- 预期：触发 equal 边界外矫正 | 剪枝到 p2_v2 单叶子
CALL check_erows('E-D-05', $$SELECT * FROM t_e_part2 WHERE ver = 51 AND region = 8 AND ver > 50$$, 200, 0.05);  -- 仅 p2_v2 满足

-- 【E-D-06】表类型 | 二级分区-不剪枝
-- 预期：触发 equal 边界外矫正 | 按 v 过滤无法剪枝
CALL check_erows('E-D-06', $$SELECT * FROM t_e_part2 WHERE ver = 51$$, 200, 0.05);  -- ver 非分区键，扫所有叶子

-- 【E-D-07】统计信息类型 | 表达式统计信息（equal 谓词在表达式列上）
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±5%）
CREATE STATISTICS st_e_expr ON (ver * 2) FROM t_e_astore;
ANALYZE t_e_astore;
CALL check_erows('E-D-07', $$SELECT * FROM t_e_astore WHERE (ver * 2) = 102$$, 200, 0.05);
DROP STATISTICS st_e_expr;
```
...
ANALYZE t_e_noincr;
-- 不再 insert
CALL check_erows('E-D-08', $$SELECT * FROM t_e_noincr WHERE ver = 51$$, 1, 0.05);

-- 【E-D-09】历史统计信息已 prune
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±容忍，走当前 relpages/reltuples）
DELETE FROM pg_statistic_history WHERE starelid = 't_e_astore'::regclass;  -- 伪代码
CALL check_erows('E-D-09', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 200, 0.05);

-- 【E-D-10】vacuum 只更表级不更列级
-- 预期：触发 equal 边界外矫正 | rows ≈ 200（±5%）
VACUUM t_e_astore;
CALL check_erows('E-D-10', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 200, 0.05);
...
CALL check_erows('E-D-11', $$SELECT * FROM t_e_ts WHERE ts = '2026-03-01'::timestamp$$, 200, 0.05);
...
CALL check_erows('E-D-12', $$SELECT * FROM t_e_bi WHERE b = 99999999999$$, 200, 0.05);
...
CALL check_erows('E-D-13', $$SELECT * FROM t_e_va WHERE v = 'ver999'$$, 200, 0.05);
```
...
CALL check_erows('S-01', $$SELECT * FROM t_r_astore WHERE id > 11000 AND id < 11100$$, 1, 0.05);
SET refine_growth_sel = on;

-- 【S-02】refine_growth_sel=off | gt 触发场景估行回退到 1
SET refine_growth_sel = off;
CALL check_erows('S-02', $$SELECT * FROM t_r_astore WHERE id > 10100$$, 1, 0.05);
SET refine_growth_sel = on;

-- 【S-03】refine_growth_sel=off | equal 触发场景估行回退到 1
SET refine_growth_sel = off;
CALL check_erows('S-03', $$SELECT * FROM t_e_astore WHERE ver = 51$$, 1, 0.05);
SET refine_growth_sel = on;

-- 【S-04】refine_growth_sel=on | 一般场景 range 估行不变（守默认开启底线）
-- 预期：rows 按直方图估算，与关闭特性时一致（±0）
CALL check_erows('S-04', $$SELECT * FROM t_r_astore WHERE id > 2000 AND id < 3000$$, 1000, 0.05);

-- 【S-05】refine_growth_sel=on | 一般场景 equal 命中 MCV 估行不变
CALL check_erows('S-05', $$SELECT * FROM t_e_astore WHERE ver = 1$$, 200, 0.05);

-- 【S-06】refine_growth_sel=on vs off | 一般场景 range 数值严格一致
-- 前置：分别断言 refine_growth_sel=off/on 的 rows 相同
SET refine_growth_sel = off;
CALL check_erows('S-06-off', $$SELECT * FROM t_r_astore WHERE id > 2000 AND id < 3000$$, 1000, 0.05);
SET refine_growth_sel = on;
CALL check_erows('S-06-on', $$SELECT * FROM t_r_astore WHERE id > 2000 AND id < 3000$$, 1000, 0.05);

-- 【S-07】refine_growth_sel=on vs off | 一般场景 gt 数值严格一致
SET refine_growth_sel = off;
CALL check_erows('S-07-off', $$SELECT * FROM t_r_astore WHERE id > 5000$$, 5000, 0.05);
SET refine_growth_sel = on;
CALL check_erows('S-07-on', $$SELECT * FROM t_r_astore WHERE id > 5000$$, 5000, 0.05);

-- 【S-08】refine_growth_sel=on vs off | 一般场景 equal 命中 MCV 数值严格一致
SET refine_growth_sel = off;
CALL check_erows('S-08-off', $$SELECT * FROM t_e_astore WHERE ver = 1$$, 200, 0.05);
SET refine_growth_sel = on;
CALL check_erows('S-08-on', $$SELECT * FROM t_e_astore WHERE ver = 1$$, 200, 0.05);

-- 【S-09】refine_growth_sel=on | equal 未命中 MCV 但 other_distinct>0 的一般场景估行不变
SET refine_growth_sel = off;
CALL check_erows('S-09-off', $$SELECT * FROM t_e_otherdist WHERE ver = 50$$, 10, 0.05);
SET refine_growth_sel = on;
CALL check_erows('S-09-on', $$SELECT * FROM t_e_otherdist WHERE ver = 50$$, 10, 0.05);
```
